...
test('Test initial displaying of all matches', () => {
    render(<MatchList {...props}></MatchList>);

    expect(screen.getAllByAltText('Team logo').length).toBe(leagues[0].teams.length + leagues[1].teams.length);
    expect(screen.getByText(leagues[0].teams[0].name)).not.toBeNull();
})

describe('Test selection of showed leagues and matches', () => {
    
    test('Test when teams unchecked', () => {
    ...
    test('Test when league unchecked', () => {
...
test('Test sorting by date and time', () => {
