...
test('Test team name and checkbox displaying', () => {
    render(<Team {...props}></Team>);

    expect(screen.queryByText(team.name)).not.toBeNull();
    expect(shallow(<Team {...props}></Team>).find('Checkbox').props().checked).toEqual(true);
});
...
describe('Test checkbox click', () => {
    ...
    test('Test function invocation after click', () => {
        render(<Team {...props}></Team>);
